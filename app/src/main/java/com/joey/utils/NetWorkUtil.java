...
import com.joey.ui.base.MainApplication;
...
public class NetWorkUtil {
    ...
    private static String getMacFromCallCmd() {
        ...
        LogUtils.v(TAG, "cmd result : " + result);
    ...
    public static String getPhoneOnlyCode() {
        ...
        if (TextUtils.equals("000000000000000", ime)) {
            ...
            if (!TextUtils.isEmpty(mac)) {
                LogUtils.e(TAG, "phone wifi mac:" + mac);
                mDeviceOnlyCode = mac;
            } else {
                LogUtils.e(TAG, "phone ime(empty):000000000000000");
                mDeviceOnlyCode = ime;
            }
        } else {
            LogUtils.e(TAG, "phone ime:" + ime);
            mDeviceOnlyCode = ime;
        }

        LogUtils.e(TAG, "mDeviceOnlyCode:" + mDeviceOnlyCode);
...
//                                .getHostAddress())
    ...
    private static boolean pingWebUrl() {
        ...
        try {
            ...
            LogUtils.v(TAG, "ping耗时:" + difftime);
            if (retCode == 0) {
                LogUtils.v(TAG, "ping连接成功");
                return true;
            } else {
                LogUtils.e(TAG, "ping测试失败");
            }
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
    ...
    private static boolean connectionWebUrl() {
        ...
        if (result) {
            LogUtils.v(TAG, "测试连接百度成功");
        } else {
            LogUtils.v(TAG, "测试连接百度失败");
        }
